...
contract Warchest is IWarchest, ControllableNoInit, SolidStateERC20, Initializable {
    ...
    function addStrategies(WarchestStorage.Strategy[] memory strategies) external onlyGovernance {
    ...
    function emergencyExit(uint256 _amountInShares) external {
        _require(WarchestStorage.layout().circuitBreakerActive == true, Errors.CIRCUIT_BREAKER_NOT_ACTIVE);
