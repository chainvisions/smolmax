...
/// @title Limestone Voting Escrow
/// @notice veNFT implementation that escrows ERC-20 tokens in the form of an ERC-721 NFT
/// @notice Votes have a weight depending on time, so that users are committed to the future of (whatever they are voting for)
/// @author Modified from Solidly (https://github.com/solidlyexchange/solidly/blob/master/contracts/ve.sol)
/// @author Modified from Curve (https://github.com/curvefi/curve-dao-contracts/blob/master/contracts/VotingEscrow.vy)
/// @author Modified from Nouns DAO (https://github.com/withtally/my-nft-dao-project/blob/main/contracts/ERC721Checkpointable.sol)
/// @author Modified from Ramses (https://arbiscan.io/address/0xAAA343032aA79eE9a6897Dab03bef967c3289a06)
    ...
    ReentrancyGuard
{
    ...
    /// @notice Data structure for veNFT voting data.
    struct VoterInfo {
        bool voted;
        uint128 attachments;
    }
    ...
    /// @notice Limestone voter contract.
    address public voter;

    /// @notice Limestone team.
    address public team;

    /// @notice Limestone veNFT art.
    ...
    string public constant name = "veLIME";
    string public constant symbol = "veLIME";
    ...
    mapping(uint256 => VoterInfo) public voterInfo;
    ...
    function tokenURI(uint256 _tokenId) external view returns (string memory) {
        _require(
            ...
            Errors.QUERIED_NONEXISTENT_TOKEN
        );
    ...
    function _transferFrom(
        ...
        VoterInfo memory _vinf = voterInfo[_tokenId];
        require(!IVoter(voter).stale(_tokenId), "STALE");
        require(_vinf.attachments == 0 && !_vinf.voted, "attached");
        // Check requirements
    ...
    function safeTransferFrom(
        ...
        _require(!IVoter(voter).stale(_tokenId), Errors.STALE);
        _transferFrom(_from, _to, _tokenId, msg.sender);
    ...
    function deposit_for(
        ...
        _require(_locked.amount > 0, Errors.NO_LOCK_FOUND);
        _require(_locked.end > block.timestamp, Errors.EXPIRED_LOCK);
        _deposit_for(
            _tokenId,
            _value,
            0,
    ...
    function _create_lock(
        ...
        require(_value > 0); // dev: need non-zero value
        _require(unlock_time > block.timestamp, Errors.CANNOT_LOCK_BACKWARDS);
        _require(
            unlock_time <= block.timestamp + MAXTIME,
            Errors.FOUR_YEARS_MAX
        );
    ...
    function increase_unlock_time(
        ...
        _require(_locked.end > block.timestamp, Errors.EXPIRED_LOCK);
        _require(_locked.amount > 0, Errors.NO_LOCK_FOUND);
        _require(unlock_time > _locked.end, Errors.CANNOT_LOCK_BACKWARDS);
        _require(
            unlock_time <= block.timestamp + MAXTIME,
            Errors.FOUR_YEARS_MAX
        );
    ...
    function withdraw(uint256 _tokenId) external nonReentrant {
        assert(_isApprovedOrOwner(msg.sender, _tokenId));
        VoterInfo memory _vinf = voterInfo[_tokenId];
        _require(_vinf.attachments == 0 && !_vinf.voted, Errors.ATTACHED);

        LockedBalance memory _locked = locked[_tokenId];
        _require(block.timestamp >= _locked.end, Errors.LOCK_NOT_EXPIRED);
    ...
    function voting(uint256 _tokenId) external {
        ...
        voterInfo[_tokenId].voted = true;
    }

    function abstain(uint256 _tokenId) external {
        require(msg.sender == voter);
        voterInfo[_tokenId].voted = false;
    }

    function attach(uint256 _tokenId) external {
        require(msg.sender == voter);
        voterInfo[_tokenId].attachments = voterInfo[_tokenId].attachments + 1;
    }
    ...
    function merge(uint256 _from, uint256 _to) external {
        VoterInfo memory _vinf = voterInfo[_from];
        _require(
    ...
    function split(
        ...
        _require(initialLocked.end > block.timestamp, Errors.EXPIRED_LOCK);
    ...
    function delegateBySig(
        ...
        _require(signatory != address(0), Errors.DBSIG_INVALID_SIG);
        _require(nonce == nonces[signatory]++, Errors.DBSIG_INVALID_NONCE);
        _require(block.timestamp <= expiry, Errors.DBSIG_EXP_SIG);
